...
class AutoGraphLearner (modelClassName: String, ctx: TaskContext) extends MLLearner(ctx) {
  ...
  def setTuneIter(iter: Int): this.type = {
    tuneIter = iter
    this
  }
  ...
  def resetParam(paramMap: mutable.Map[String, Double]): this.type = {
    ...
    epochNum = paramMap.getOrElse(MLConf.ML_EPOCH_NUM, epochNum.toDouble).toInt
    println(s"numEpoch=$epochNum")
    this
  }
