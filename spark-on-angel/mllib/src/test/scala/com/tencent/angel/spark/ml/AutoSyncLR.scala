...
class AutoSyncLR extends PSFunSuite with SharedPSContext {
  ...
  override def beforeAll(): Unit = {
    ...
    input = "../../data/agaricus/agaricus_127d_train.libsvm"
    ...
    SharedConf.get().setDouble(MLConf.ML_LEARN_RATE, 0.001)
    SharedConf.get().setDouble(MLConf.ML_LEARN_DECAY, 0.01)
    SharedConf.get().setDouble(MLConf.ML_REG_L2, 0.01)
    ...
    learner = new AutoSyncLearner(5,false)
