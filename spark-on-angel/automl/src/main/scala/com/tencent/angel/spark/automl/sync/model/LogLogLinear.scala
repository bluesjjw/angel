...
/**
  * y = log(a * log(x) + b)
  */
class LogLogLinear extends PowerLaw {

  override val numParams: Int = 2
  override val params: BDV[Double] = BDV(1.0, 1.0)

  override def predict(x: Double): Double = {
    math.log10(params(0) * math.log10(x) + params(1))
  }
  ...
  override def grad(x: Double): BDV[Double] = {
    val tmp = 1 / (params(0) * math.log10(x) + params(1))
    BDV(Array(tmp * math.log10(x), tmp))
  }
...
object LogLogLinear {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val initParams = BDV(1.0, 1.0)
    println(s"initial params: $initParams")
    val model = new LogLogLinear().setParams(initParams)
